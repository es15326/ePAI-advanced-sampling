...
from nnunetv2.inference.export_prediction import parse_prediction_new_row

def multi_processing_parsing_groud_truth_for_panc_info_row(
        cases, args, dataset_json_dict_for_panc_duct_tumors, pancreas_and_tumor_labels, num_processes=12):
        print(f">> finding {len(cases)} cases, starting...")
        print('>> {} CPU cores are secured.'.format(int(num_processes)))
        print(">> Parsing ground truth meta information and add to result csv...")
        results = []
        with ProcessPoolExecutor(max_workers=int(num_processes)) as executor:
            futures = {executor.submit(parse_ground_truth_for_panc_info_row, 
                                (case, args, dataset_json_dict_for_panc_duct_tumors, pancreas_and_tumor_labels)): 
                                case for case in cases}
            for future in tqdm(as_completed(futures), total=len(futures), desc="Parsing GT"):
                case = futures[future]
                try:
        ...
        #     print(res)
        return results
def parse_ground_truth_for_panc_info_row(inputs):
        bdmap_id, args, dataset_json_dict_for_panc_duct_tumors, pancreas_and_tumor_labels = inputs
        label_file_path = join(args.l, bdmap_id + ".nii.gz") 
        combined_label_nii = nib.load(label_file_path)
        combined_label = combined_label_nii.get_fdata()
        hwd_shape = combined_label_nii.shape
        dhw_shape = (hwd_shape[2], hwd_shape[0], hwd_shape[1])  # HWD to DHW
        hwd_spacing = combined_label_nii.header.get_zooms()
        dhw_spacing = (hwd_spacing[2], hwd_spacing[0], hwd_spacing[1])  # HWD to DHW
        properties_dict_shape_and_spacing = dict(shape_before_cropping=dhw_shape, spacing=dhw_spacing)
        # segmentation_final = combined_label
        ...
        # combined_label = segmentation_final
        _, _, _, \
        panc,panc_component_count,panc_voxel_size,panc_volume_size, \
        duct,duct_component_count,duct_voxel_size,duct_volume_size, \
        PDAC,PDAC_component_count,PDAC_voxel_size,PDAC_volume_size, _, \
        cyst,cyst_component_count,cyst_voxel_size,cyst_volume_size, _, \
        PNET,PNET_component_count,PNET_voxel_size,PNET_volume_size, _ = \
            parse_prediction_new_row(
                pred_pancreas_tumor=combined_label, 
                probabilities_final=None, 
                properties_dict=properties_dict_shape_and_spacing, 
                output_file_truncated=bdmap_id, 
                dataset_json_dict_or_file=dataset_json_dict_for_panc_duct_tumors)
        gt_info_row = [
            panc,panc_component_count,panc_voxel_size,panc_volume_size,
            duct,duct_component_count,duct_voxel_size,duct_volume_size,
            PDAC,PDAC_component_count,PDAC_voxel_size,PDAC_volume_size,
            cyst,cyst_component_count,cyst_voxel_size,cyst_volume_size,
            PNET,PNET_component_count,PNET_voxel_size,PNET_volume_size,
        ]
        # print(gt_info_row, len(gt_info_row))
        return bdmap_id, gt_info_row
...
class nnUNetPredictor(object):
    ...
    def _resume_list_of_lists_from_csv(self, list_of_lists, csv_path, output_csv_path):
        ...
        for idx, bdmap_id in enumerate(input_ct_bdmap_id_list):
            try: 
                ...
                continue
        print(f"\033[31m[FlagshipToCSV][INFO] The current part of job has {len(list_of_lists)} cases in total. But with respect to the {csv_path}, we only work on {len(list_of_lists_new)}.\033[0m")
    ...
    def _manage_input_and_output_lists(self, list_of_lists_or_source_folder: Union[str, List[List[str]]], csv_path: str, output_csv_path: str, 
        ...
        if isinstance(list_of_lists_or_source_folder, str):
            list_of_lists_or_source_folder = create_lists_from_splitted_dataset_folder(list_of_lists_or_source_folder,
                                                                                       self.dataset_json['file_ending'], num_processes=24)
        print(f'There are {len(list_of_lists_or_source_folder)} cases in the source folder')
        list_of_lists_or_source_folder = list_of_lists_or_source_folder[part_id::num_parts]
        list_of_lists_or_source_folder = self._resume_list_of_lists_from_csv(list_of_lists_or_source_folder, csv_path, output_csv_path)  # NOTE: resume from csv!!!
        ...
        if isinstance(output_folder_or_list_of_truncated_output_files, str):    # ePAI will run this
            ...
            output_filename_truncated = self._resume_list_of_lists_from_csv(output_filename_truncated, csv_path, output_csv_path)  # just to be sure, may never use this
...
def predict_entry_point_modelfolder():
    ...
    parser.add_argument('-l', type=str, required=False,
                        help='folder of combined labels. Remember to use the correct channel numberings for your files (_0000 etc). '
                             'File endings must be the same as the training dataset!')
    parser.add_argument('--panc_duct_pdac_cyst_pnet', type=int, required=False, nargs='+', help='label id in the exact same order!!!')
    ...
    parser.add_argument('--add_gt_info_to_csv', action='store_true', help="Wehther to add gt info after all the inference has done.")
    ...
    # NOTE: The inference process is here!
    # Pedro: avoiding predicting from args.i, this is too slow! we can quickly build a list of files with os.listdir.
    input_folder = args.i
    file_ending = '.nii.gz'  # e.g. ".nii.gz"
    cases = sorted(f for f in os.listdir(input_folder) if f.endswith(file_ending))
    list_of_lists = [[ os.path.join(input_folder, f) ] for f in cases]
    ...
    # NOTE: Check if there are still empty cases in the result csv. Normally it shouldn't (when num_part==1)
    df_out = pd.read_csv(args.output_csv)
    try:
        assert len(df_out) == len(df_out.dropna()), f"There are {len(df_out)} cases in the result csv, but only {len(df_out.dropna())} has finished! ({len(df_out)-len(df_out.dropna())} missing cases)"
    except AssertionError as e:
        print(f"Error message: {e}")
        print(f"Meaning: there are still empty rows in the result csv file! This could be casued by mainly 3 reasons:")
        print(f"    1. If you're using multi-parts to inference, the other parts might not yet finished. No worries! Just wait for the last part to finish!")
        print(f"    2. Check if your dataset folder covers all the cases in the csv file! If not, please fill those missing CT scans and run the inference again!")
        print(f"    3. If both 1-st/2-nd reasons are not your cases, ah oh, then there might be something wrong with multi-processing/multi-threading code.")
        print(f"        - Normally, just simply re-run the inference code will fill up the missing cases.")
        print(f"        - And please report this issue to us!")
        import sys; sys.exit(0) # NOTE: this is not an error for most cases, so we are finish the code rather than raising an error.
    print("\033[31mInference finished!\033[0m")
    
    # NOTE: add the ground truth information to the result csv!
    if args.add_gt_info_to_csv:
        # NOTE: parse the ground truth label and add the corresponding information to the result csv!
        final_column_list = pd.read_csv("csv_header.csv").columns.tolist()  # with the correct order
        num_already_had_info_column = len(df_out.columns)
        for column_name in final_column_list:
            if "gt" in column_name and column_name not in df_out.columns.tolist():
                df_out[column_name] = None
            elif "gt" in column_name and column_name in df_out.columns.tolist():
                # if there is any "gt" in the result csv, it means that the following code has been run (already has gt info).
                print("Seems that GT information is already in the result csv! Bye!")
                import sys; sys.exit(0)
            else:
                continue
        dataset_json_dict_for_panc_duct_tumors = dict(labels=dict(
            pancreas=args.panc_duct_pdac_cyst_pnet[0], #13,
            pancreatic_duct=args.panc_duct_pdac_cyst_pnet[1], #14,
            pancreatic_pdac=args.panc_duct_pdac_cyst_pnet[2], #23,
            pancreatic_cyst=args.panc_duct_pdac_cyst_pnet[3], #24,
            pancreatic_pnet=args.panc_duct_pdac_cyst_pnet[4], #25
        ))
        pancreas_and_tumor_labels = [
                dataset_json_dict_for_panc_duct_tumors["labels"]["pancreas"],
                dataset_json_dict_for_panc_duct_tumors["labels"]["pancreatic_duct"],
                dataset_json_dict_for_panc_duct_tumors["labels"]["pancreatic_pdac"],
                dataset_json_dict_for_panc_duct_tumors["labels"]["pancreatic_cyst"],
                dataset_json_dict_for_panc_duct_tumors["labels"]["pancreatic_pnet"]
        ]
        results_list = multi_processing_parsing_groud_truth_for_panc_info_row(
            df_out["bdmap_id"], args, dataset_json_dict_for_panc_duct_tumors, pancreas_and_tumor_labels, num_processes=12)
        for bdmap_id, gt_info_row in results_list:
            writeable_gt_info_row = tuple(map(lambda x: str(x) if isinstance(x, list) or isinstance(x, tuple) else x, gt_info_row)) # turn list and tuple to str
            row_index = df_out["bdmap_id"].tolist().index(bdmap_id)
            df_out.iloc[row_index, num_already_had_info_column:] = writeable_gt_info_row
        df_out = df_out[final_column_list]
        df_out.to_csv(args.output_csv, index=False)
        print("\033[31mGround Truth information added to the result csv!\033[0m")
