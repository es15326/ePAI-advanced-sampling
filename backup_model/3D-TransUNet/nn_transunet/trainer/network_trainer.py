...
class NetworkTrainer(object):
    ...
    def load_latest_checkpoint(self, train=True, finetune=False):
        print('self.output_folder', self.output_folder)
        if isfile(join(self.output_folder, "model_final_checkpoint.model")):
            return self.load_checkpoint(join(self.output_folder, "model_final_checkpoint.model"), train=train, finetune=finetune)
    ...
    def load_checkpoint(self, fname, train=True, finetune=False):
        self.print_to_log_file("loading checkpoint", fname, "train=", train)
        if not self.was_initialized:
            self.initialize(train)
        # saved_model = torch.load(fname, map_location=torch.device('cuda', torch.cuda.current_device()))
        saved_model = torch.load(fname, map_location=torch.device('cpu'))
        self.load_checkpoint_ram(saved_model, train, finetune)
