...
def export_prediction_from_logits(predicted_array_or_file: Union[np.ndarray, torch.Tensor], properties_dict: dict,
    ...
    print("\n\n--- USING THE MODIFIED export_prediction.py FILE ---\n\n")
    ...
    if save_probabilities:
        ...
        print(dataset_json_dict_or_file["labels"])
        print('#################################')
        
        # --- MODIFIED ---
        # Re-enabled standard nnU-Net probability saving.
        np.savez_compressed(output_file_truncated + '.npz', probabilities=probabilities_final)
        save_pickle(properties_dict, output_file_truncated + '.pkl')

        # --- MODIFIED ---
        # The entire custom logic block below has been commented out to prevent crashes
        # on datasets that do not conform to the specific 5-label pancreas schema.
        #
    ...
    # --- MODIFIED ---
    # Re-enabled the standard nnU-Net function to save the final segmentation file.
    # This ensures that a .nii.gz file is always created as the output.
    rw = plans_manager.image_reader_writer_class()
    rw.write_seg(segmentation_final, output_file_truncated + dataset_json_dict_or_file['file_ending'],
                 properties_dict)
