...
#
class nnUNetTrainer_LesionPatchSampler(nnUNetTrainer):
    """
    ...
    - NNUNET_NEG_SAMPLES: (int) Number of negative samples per epoch.
    - NNUNET_FG_OVERSAMPLE: (float) The oversample_foreground_percent (patch-level).
    - NNUNet_PATCH_OVERSAMPLE_LESION_ONLY: (0 or 1) If 1, restricts patch-level oversampling to only the target_label_id.
    """
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        try:
            patch_lesion_only_str = os.environ.get('NNUNET_PATCH_OVERSAMPLE_LESION_ONLY', str(default_patch_lesion_only))
            self.patch_oversample_lesion_only = bool(int(patch_lesion_only_str))
        except ValueError:
            print(f"Warning: Invalid NNUNET_PATCH_OVERSAMPLE_LESION_ONLY. Using default: {default_patch_lesion_only}")
            self.patch_oversample_lesion_only = bool(default_patch_lesion_only)
            patch_lesion_only_str = str(default_patch_lesion_only)

        # --- MODIFY OUTPUT FOLDER ---
        fg_oversample_suffix = fg_oversample_str.replace('.', 'p')
        patch_lesion_suffix = "_L_Only" if self.patch_oversample_lesion_only else ""
        
        experiment_suffix = f"_fg{fg_oversample_suffix}_neg{neg_samples_str}{patch_lesion_suffix}"
        ...
        if self.target_label_id is not None:
            self.print_to_log_file("--- LesionPatchSampler Configuration ---") 
            ...
            patch_sampling_mode = "Lesion-Only" if self.patch_oversample_lesion_only else "All Foreground Classes"
            self.print_to_log_file(f"Patch-Level Sampling Mode: {patch_sampling_mode} (from NNUNET_PATCH_OVERSAMPLE_LESION_ONLY)")
    ...
    # ====================================================================================
    # Overridden Methods (Lifecycle management - NO CHANGES)
    # ====================================================================================
    ...
    def get_dataloaders(self):
        ...
        original_foreground_labels = self.label_manager.foreground_labels
        original_foreground_regions = self.label_manager.foreground_regions if self.label_manager.has_regions else None

        tr_transforms = self.get_training_transforms(
            patch_size, rotation_for_DA, deep_supervision_scales, mirror_axes, do_dummy_2d_data_aug,
            use_mask_for_norm=self.configuration_manager.use_mask_for_norm,
            is_cascaded=self.is_cascaded, foreground_labels=original_foreground_labels,
            regions=original_foreground_regions,
            ignore_label=self.label_manager.ignore_label)

        val_transforms = self.get_validation_transforms(deep_supervision_scales,
                                                        is_cascaded=self.is_cascaded,
                                                        foreground_labels=original_foreground_labels,
                                                        regions=original_foreground_regions,
                                                        ignore_label=self.label_manager.ignore_label)
        ...
        if self.patch_oversample_lesion_only:
            if self.target_label_id is None:
                self.print_to_log_file("WARNING: Cannot restrict patch oversampling, target_label_id not found.")
            else:
                self.print_to_log_file(f"RESTRICTING patch-level oversampling to target label: {self.target_label_name} (ID: {self.target_label_id})")
                
                if hasattr(self.dl_tr_raw, '_patch_generator') and hasattr(self.dl_tr_raw._patch_generator, 'foreground_classes'):
                    self.dl_tr_raw._patch_generator.foreground_classes = (self.target_label_id,)
                else:
                    self.print_to_log_file("WARNING: Could not find _patch_generator.foreground_classes. Patch oversampling might not be restricted.")
